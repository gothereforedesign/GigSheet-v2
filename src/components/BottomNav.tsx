...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-30 bg-white/95 backdrop-blur-md border-t border-slate-200/80 shadow-lg select-none">
      <div className="max-w-md mx-auto flex items-center justify-between h-16 px-4 gap-2">
        ...
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center py-2 px-3 rounded-2xl transition-all cursor-pointer ${
            isSheetMusicSection
              ? 'text-[#0c4a6e] font-black bg-sky-50/80'
        ...
        <button
          ...
          className={`p-3 rounded-2xl text-white transition-all cursor-pointer active:scale-95 shadow-md flex items-center justify-center shrink-0 mx-1 ${
            isTechniqueSection
              ? 'bg-purple-900 hover:bg-purple-950 shadow-purple-900/20'
              : 'bg-[#0c4a6e] hover:bg-[#073652] shadow-sky-900/20'
          }`}
          title="Add PDF Chart"
        >
        ...
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center py-2 px-3 rounded-2xl transition-all cursor-pointer ${
            isTechniqueSection
              ? 'text-purple-900 font-black bg-purple-50/80'
              : 'text-slate-400 hover:text-slate-600 font-semibold hover:bg-slate-50'
          }`}
        >
          <div className="flex flex-col items-center">
