    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-slate-200 flex flex-col">
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="text-sm font-black uppercase tracking-wider truncate leading-tight">
        ...
        <div className="p-4 space-y-3 max-h-[60vh] overflow-y-auto">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl border flex items-center justify-between transition-all cursor-pointer ${
                      isChecked
                        ? isTechnique
                          ? 'bg-purple-50 border-purple-300 text-purple-950 shadow-2xs font-black'
                          : 'bg-sky-50 border-sky-300 text-[#0c4a6e] shadow-2xs font-black'
          ...
          {/* Inline Create Form */}
          {isCreatingInline ? (
            <form onSubmit={handleCreateAndSelect} className="pt-2 border-t border-slate-100 space-y-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="flex-1 p-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold outline-none text-slate-800"
                />
                <button
                  type="submit"
                ...
                <button
                  ...
                  className="px-2 py-2 text-slate-400 hover:text-slate-600 text-xs font-bold cursor-pointer"
                >
                  Cancel
                </button>
              </div>
            ...
            sectionSetlists.length > 0 && (
              <button
                type="button"
                onClick={() => setIsCreatingInline(true)}
                className="w-full text-left py-2 px-1 text-xs font-extrabold text-sky-600 hover:text-sky-800 flex items-center gap-1 cursor-pointer transition-colors"
              >
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-2">
          <p className="text-[10px] font-medium text-slate-500 max-w-[200px] leading-tight">
