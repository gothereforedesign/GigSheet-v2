  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-80 sm:w-96 bg-white border border-slate-200/90 shadow-2xl rounded-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-5 duration-200">
      ...
      {quotaError && !isMinimized && (
        <div className="p-4 bg-rose-50 border-b border-rose-100 flex items-start gap-3">
          ...
          <div className="space-y-1">
            <p className="text-xs font-bold text-rose-900">Upload Halting</p>
            <p className="text-[11px] text-rose-700 leading-relaxed">{quotaError}</p>
          </div>
        </div>
      )}
      ...
      {!isMinimized && items.length > 0 && (
        <div className="flex flex-col max-h-72 bg-slate-50/50">
          <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between bg-white text-[11px] font-bold text-slate-500">
            <span>Batch Upload Queue</span>
            {allFinished && (
              <button
                type="button"
                onClick={onClearCompleted}
                className="text-sky-600 hover:text-sky-800 font-extrabold flex items-center gap-1 cursor-pointer"
              >
